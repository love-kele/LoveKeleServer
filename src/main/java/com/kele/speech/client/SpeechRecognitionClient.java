...
public class SpeechRecognitionClient {
    ...
    public BaiduSdkResponse voiceToText(SpeechRequest speechRequest) {
       ...
       // options.put(Constants.Common.DEV_PID, Constants.LangType.PuTongHua_Model_Input);
        JSONObject jsonObject = aipSpeech.asr(speechRequest.getData(),
                speechRequest.getFormat(), Constants.Common.rate, options);
        BaiduSdkResponse baiduSdkResponse = GsonUtil.jsonToBean(String.valueOf(jsonObject),
                BaiduSdkResponse.class);
